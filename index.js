...
function epi(totalWin, times, total, xy){
  ...
  Object.assign(saveArray, mainArray);
  return [totalWin+win, times+1, xy[0], xy[1], ucb(totalWin+win, times+1, total)];
}
...
function best(){
    ...
    for(var j = -1; j < 2 && unluckypoint == 0 ; j++){for(var k = -1; k < 2 && unluckypoint == 0; k++){
      for(var l = 0; l < luckypoint.length && unluckypoint == 0; l++){
        if((j != 0 || k != 0) && max[0]+j == luckypoint[l][0] && max[1]+k == luckypoint[l][1]){
          if(sphere[luckypoint[l][0]][luckypoint[l][1]].color == maincolor){//すでに角をとっているなら置いたほうが良い
            unluckypoint = -1;
            break;
          }
  ...
  //最適な手段を選択
  return [hopebest[0][2], hopebest[0][3]];
}
...
function zodiac(){
  ...
  //九局面までsemimanabukun
  if(step < 10) xy = semimanabukun(0.1, 0);
...
function mainAR(){
  ...
  for(var i = 0; i < N; i++){
    mainArray[i] = [];
    for(var j = 0; j < N; j++) mainArray[i][j] = sphere[i][j].color;
  }
  return mainArray;
}
